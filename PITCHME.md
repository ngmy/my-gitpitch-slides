...
- PC・スマホから24時間いつでも注文できて、自宅にいたまま預けや受け取りができるWebサービス
- 衣類クリーニング
    - https://www.lenet.jp/
- 衣類クリーニング保管
...
1つのモノリシックなアプリケーション内をドメインごとにモジュールに分解して運用するアーキテクチャ
...
- サービスごとを疎結合にしつつ共通のコードを共有できる
- ドメイン上のよい境界を見つけ、アプリ内をモジュールに分解していき、ゆくゆくはマルチリポジトリを目指す（かも？）
...
- 各サービスの設定ファイルは共有コードの独自ヘルパーを使って読み込んでいる
- Laravelの`config`関数は使っていない
